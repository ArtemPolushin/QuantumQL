...
def test_apply_with_register_target():
    ast = Program([
...
def test_apply_with_range_target():
    ast = Program([
...
def test_apply_with_wildcard_target():
    ast = Program([
...
def test_apply_multiple_targets_mixed():
    ast = Program([
        ApplyGate("BARRIER", [
            QubitRef("q", 0),
            QubitRef("q", (1, 3)),
...
def test_apply_with_number_param():
    ast = Program([
...
def test_apply_with_float_param():
    ast = Program([
...
def test_apply_with_multiple_number_params():
    ast = Program([
...
def test_apply_with_variable_param():
    ast = Program([
...
def test_apply_with_constant_param():
    ast = Program([
...
def test_apply_with_binary_expression_param():
    ast = Program([
...
def test_apply_with_complex_expression_param():
    ast = Program([
        ApplyGate("RX", [QubitRef("q", 0)], [
            BinOp(
...
def test_apply_with_unary_expression_param():
    ast = Program([
        ApplyGate("RX", [QubitRef("q", 0)], [
            UnaryOp("-", Number(3.14))
        ])
...
def test_apply_with_function_call_param():
    ast = Program([
        ApplyGate("RX", [QubitRef("q", 0)], [
...
def test_apply_with_mixed_param_types():
    ast = Program([
        ApplyGate("U3", [QubitRef("q", 0)], [
            Number(1.0),
            Var("theta"),
...
def test_measure_multiple_qubits():
    ast = Program([
        Measure([
            QubitRef("q", 0),
            QubitRef("q", 1),
...
def test_gate_definition_simple():
    ast = Program([
        GateDef("MY_GATE", ["a", "b"], [
...
def test_gate_definition_with_params():
    ast = Program([
...
def test_gate_definition_multiple_statements():
    ast = Program([
...
def test_nested_gate_definitions():
    ast = Program([
        GateDef("OUTER", ["a"], [
            GateDef("INNER", ["b"], [
...
def test_select_statement_without_condition():
    ast = Program([
...
def test_select_statement_with_complex_condition():
    ast = Program([
...
def test_apply_with_mixed_targets():
    ast = Program([
        ApplyGate("BARRIER", [
            QubitRef("q", 0),
            SelectExpr("s", "a", None),
...
def test_apply_with_all_from_select():
    ast = Program([
        ApplyGate("H", [AllFromSelect("even")], [])
    ])
    ...
    assert len(stmt.targets) == 1
    target = stmt.targets[0]
    assert isinstance(target, IRAggregateAlias)
    assert target.alias == "even"

def test_apply_with_all_from_where():
    ast = Program([
        ApplyGate("X", [
            AllFromWhere("q", BinOp(Var("index"), ">=", Number(2.0)))
        ], [])
    ])
    ...
    assert len(stmt.targets) == 1
    target = stmt.targets[0]
    assert isinstance(target, IRAggregateTarget)
    assert target.source == "q"
    assert isinstance(target.condition, IRBinOp)
    assert target.condition.op == ">="

def test_apply_with_all_from_where_no_condition():
    ast = Program([
        ApplyGate("X", [
            AllFromWhere("q", None)
        ], [])
...
def test_apply_with_mixed_targets_including_all_from():
    ast = Program([
        ApplyGate("BARRIER", [
            QubitRef("q", 0),
            AllFromSelect("even"),
            AllFromWhere("q", BinOp(Var("index"), ">", Number(5.0)))
        ], [])
    ])
    ...
    assert len(stmt.targets) == 3
    assert isinstance(stmt.targets[0], IRQubit)
    assert isinstance(stmt.targets[1], IRAggregateAlias)
    assert isinstance(stmt.targets[2], IRAggregateTarget)

def test_measure_with_all_from():
    ast = Program([
        Measure(
            [AllFromSelect("selected")],
            [QubitRef("c", 0)]
        )
    ])
    ...
    assert isinstance(stmt, IRMeasure)
    assert len(stmt.source) == 1
    assert isinstance(stmt.source[0], IRAggregateAlias)
    assert stmt.source[0].alias == "selected"
    assert len(stmt.target) == 1
    assert isinstance(stmt.target[0], IRQubit)
...
def test_input_param():
    ...
    stmt = ir.body[0]
    assert isinstance(stmt, IRInputParam)
    assert stmt.name == "theta"

def test_input_multiple_params():
    ast = Program([
        InputParam("alpha"),
        InputParam("beta"),
        InputParam("gamma")
    ...
    assert len(ir.body) == 3
    assert all(isinstance(s, IRInputParam) for s in ir.body)
    assert [s.name for s in ir.body] == ["alpha", "beta", "gamma"]
...
def test_create_bits():
    ...
    stmt = ir.body[0]
    assert isinstance(stmt, IRCreateBits)
    assert stmt.name == "c"
    assert stmt.size == 8

def test_create_bits_multiple():
    ast = Program([
        CreateBits("c1", 4),
        CreateBits("c2", 8)
    ])
    ...
    assert len(ir.body) == 2
    assert isinstance(ir.body[0], IRCreateBits)
    assert isinstance(ir.body[1], IRCreateBits)
    assert ir.body[0].name == "c1"
    assert ir.body[0].size == 4
    assert ir.body[1].name == "c2"
    assert ir.body[1].size == 8

def test_apply_with_select_target_no_condition():
    ast = Program([
        ApplyGate("H", [
            SelectExpr("s", "q", None)
        ], [])
    ...
    target = stmt.targets[0]
    assert isinstance(target, IRSelectTarget)
    assert target.alias == "s"
    assert target.source == "q"
    assert target.condition is None

def test_qubit_ref_with_tuple_index():
    ast = Program([
        ApplyGate("H", [QubitRef("q", (0, 3))], [])
    ])
    ...
    target = stmt.targets[0]
    assert target.reg == "q"
    assert target.index == (0, 3)

def test_qubit_ref_with_string_wildcard():
    ast = Program([
        ApplyGate("X", [QubitRef("q", "*")], [])
    ])
...
def test_apply_with_function_call_multiple_args():
    ast = Program([
        ApplyGate("U", [QubitRef("q", 0)], [
            FuncCall("pow", [Number(2.0), Number(3.0)])
        ])
    ])
    ...
    param = stmt.params[0]
    assert isinstance(param, IRFuncCall)
    assert param.name == "pow"
    assert len(param.args) == 2
    assert param.args[0].value == 2.0
    assert param.args[1].value == 3.0


def test_select_all_and_apply_all_from():
    ast = Program([
        CreateQubits("q", 4),
        SelectStmt("all_qubits", "q", None),
        ApplyGate("H", [AllFromSelect("all_qubits")], [])
    ])
    ...
    target = ir.body[2].targets[0]
    assert isinstance(target, IRAggregateAlias)
    assert target.alias == "all_qubits"
